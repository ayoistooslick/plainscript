...
function parse(tokens) {
  ...
  function tryParseComparisonOperator(left, required) {
    ...
    // is equal to <expr> / is equal <expr>
    if (peek().type === TOKEN.IDENTIFIER && peek().value === 'equal') {
      advance();
      if (peek().type === TOKEN.TO || (peek().type === TOKEN.IDENTIFIER && peek().value === 'to')) {
        advance();
  ...
  function parseStatementCore() {
    ...
    // Natural intent repeat loops (counted, repeat with, repeat while, repeat until)
    ...
    // v1.0.363  -  every frame … done: requestAnimationFrame loop. "every" lexes
    ...
    if (token.type === TOKEN.IDENTIFIER || isSqlWordIdentifier(token)) {
      ...
      // v1.0.363  -  every frame … done: requestAnimationFrame loop. Identifier
      ...
      // v1.0.363  -  after <n> <unit>s … done: one-shot delayed execution.
...
function parseAsk() {
  ...
  function parseWhen() {
    ...
    if (!isEventWhen) {
      // v1.0.363  -  when <target> "<event>" happens [as <name>] … done binds a
...
// reply file "<path>"
  ...
  function parsePrimary() {
    ...
    // `count of <collection>` returns collection count/length
    ...
    if (item) return item;
    // v1.0.363  -  `new` works anywhere an expression is parsed (assignment
...
// atom → STRING | NUMBER | true | false | null | undefined | BigInt | '(' expr ')' |
  ...
  // v1.0.363  -  every frame … done: a requestAnimationFrame loop whose body runs
  ...
  // v1.0.363  -  after <n> <unit>s … done: run work once after a delay.
  //   after 5 seconds
  //       show "butter!"  ...  done
