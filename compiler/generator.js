...
function ensureTypeRuntime(context) {
  ...
  context.pendingPrelude.push([
    ...
    '  if (spec.kind === "dictionary") return value instanceof Map ? [...value.values()].every((v, i) => __plainMatchesType(v, spec.value, `${path}.value${i}`)) : value !== null && typeof value === "object" && !Array.isArray(value) && Object.entries(value).every(([k, v]) => __plainMatchesType(v, spec.value, `${path}.${k}`));',
    '  if (spec.kind === "map") return value instanceof Map && [...value.entries()].every(([k, v], i) => __plainMatchesType(k, spec.key, `${path}.key${i}`) && __plainMatchesType(v, spec.value, `${path}.value${i}`));',
    '  if (spec.kind === "set") return value instanceof Set && [...value].every((v, i) => __plainMatchesType(v, spec.value, `${path}[${i}]`));',
    '  if (spec.kind === "tuple") return Array.isArray(value) && value.length === spec.values.length && spec.values.every((s, i) => __plainMatchesType(value[i], s, `${path}[${i}]`));',
